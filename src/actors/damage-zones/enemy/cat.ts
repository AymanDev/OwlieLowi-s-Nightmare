...
export class Cat extends Actor {
  ...
  private _leftPaw = new CatPaw({ x: -87, y: 181, z: 1 });
  private _rightPaw = new CatPaw({ x: 93, y: 181, z: 1, scale: vec(-1, 1) });
  ...
  playMeowAnimation() {
    ...
    Logger.getInstance().info('Playing meow animation on cat');
  }
  ...
  updateTimers(engine: Game) {
    ...
    this._movementTimer = new Timer({
      ...
      interval: 3000 / engine.gameZone.currentDifficulty,
      repeats: true
    });
    ...
    this._attackTimer = new Timer({
      fcn: () => {
        Resources.EnemyAttackSfx.play(0.15);
        this.playMeowAnimation();
